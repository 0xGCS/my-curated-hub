...
export default function AIPage() {
  const { data, loading, error } = useCsvData("/data/crypto.csv");

  // Filter for AI category - checking both category field and data source
  // In the meantime, show crypto data since AI data not yet uploaded
  const aiData = data.filter(
    (row) => row.category?.toLowerCase() === "ai"
  );
  ...
  return (
    <Layout>
      <div className="container py-8">
        <div className="mb-8">
          <h1 className="font-serif text-3xl sm:text-4xl font-medium mb-2">AI</h1>
          <p className="text-muted-foreground">
            Artificial intelligence researchers, developers, companies, and educational resources.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-muted-foreground">
            <p className="text-lg mb-2">No AI data available yet</p>
            <p className="text-sm">Check back soon or add AI resources to the compendium</p>
          </div>
        ) : (
          <DataTable
            data={aiData}
